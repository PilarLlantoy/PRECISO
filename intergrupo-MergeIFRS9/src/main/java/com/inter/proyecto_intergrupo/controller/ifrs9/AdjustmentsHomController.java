...
@Controller
public class AdjustmentsHomController {

    private static final int PAGINATIONCOUNT=12;

    @Autowired
    private AdjustmentsHomService adjustmentsService;

    @Autowired
    private UserService userService;

    @GetMapping(value="/ifrs/adjustmentsHom")
    public ModelAndView showAdjustments(@RequestParam Map<String, Object> params) {
        ModelAndView modelAndView = new ModelAndView();
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        User user = userService.findUserByUserName(auth.getName());
        if(userService.validateEndpointVer(user.getId(),"Ver Ajustes Homogeneizacion")) {
            ...
            List<AdjustmentsHom> adjustments = adjustmentsService.getManuals(todayString);

            int start = (int) pageRequest.getOffset();
            int end = Math.min((start + pageRequest.getPageSize()), adjustments.size());
            Page<AdjustmentsHom> pageManuals = new PageImpl<>(adjustments.subList(start, end), pageRequest, adjustments.size());
            ...
            if (adjustments != null) {
                modelAndView.addObject("allAdj", pageManuals.getContent());
                modelAndView.addObject("current", page + 1);
                modelAndView.addObject("next", page + 2);
                modelAndView.addObject("prev", page);
                modelAndView.addObject("last", totalPage);
                modelAndView.addObject("directory", "adjustmentsHom");
                modelAndView.addObject("registers",adjustments.size());
            }

            modelAndView.addObject("period", todayString);
            modelAndView.setViewName("/ifrs/adjustmentsHom");
        }
        else
        {
    ...
    @RequestMapping(value = "/ifrs/adjustmentsHom/download")
    @ResponseBody
    public void exportAll(HttpServletResponse response, RedirectAttributes redirectAttrs, @RequestParam Map<String, Object> params) {
        try {
            ...
            String headerValue = "attachment; filename=AjustesHom_" + currentDateTime + ".xlsx";
            String todayString="";
            if(params.get("period")!=null && params.get("period").toString()=="") {
                Date today = new Date();
                Calendar calendar = Calendar.getInstance();
            ...
            List<AdjustmentsHom> manualList = adjustmentsService.getManualsDate(todayString);
            AdjustmentsHomListReport listReport = new AdjustmentsHomListReport(manualList);
    ...
    @PostMapping(value="/ifrs/adjustmentsHom/upload")
    public ModelAndView uploadFile(HttpServletRequest request, HttpServletResponse response, @RequestParam(name = "period") String period, @RequestParam Map<String, Object> params, ModelMap model) throws ParseException {
        ...
        ModelAndView modelAndView = new ModelAndView("redirect:/ifrs/adjustmentsHom");
        ...
        try{
            ...
            ArrayList<String[]> list = adjustmentsService.saveFileBD(fileContent, user, todayString);
            String[] part = list.get(0);

            if(part[0] != null && part[0].equals("Fallo Estructura") ){
                modelAndView.addObject("resp","formatAdj");
                modelAndView.addObject("period", params.get("period").toString());
                ...
                if(part[2].equals("true")){
                    modelAndView.addObject("resp", "adjCorrect");
                    modelAndView.addObject("period", params.get("period").toString());
                }else{
    ...
    @GetMapping(value = "/ifrs/clearAdjustmentsHom")
    public ModelAndView clearManuals(HttpServletResponse response, RedirectAttributes redirectAttrs, @RequestParam Map<String, Object> params){
        ...
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        User user = userService.findUserByUserName(auth.getName());
        adjustmentsService.clearManuals(user, todayString);
        ModelAndView modelAndView = new ModelAndView("redirect:/ifrs/adjustmentsHom");
        return  modelAndView;
    }
