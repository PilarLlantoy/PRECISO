...
public class Intergrupo1ListReport {
    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private XSSFSheet sheet1;
    private List<IntergrupoV1> interList;

    public Intergrupo1ListReport(List<IntergrupoV1> interList) {
        workbook = new XSSFWorkbook();
        this.interList = interList;
    }

    private void writeHeaderLine(List<IntergrupoV1> intListN){
        sheet = workbook.createSheet("Integrupo V1");
        ...
        if(intListN != null)
        {
            sheet1 = workbook.createSheet("No Consolida");
            Row row1 = sheet1.createRow(0);

            createCell(row1, 0, "YNTP Empresa Reportante", style);
            createCell(row1, 1, "Cod Neocon", style);
            createCell(row1, 2, "Divisa", style);
            createCell(row1, 3, "Yntp", style);
            createCell(row1, 4, "Sociedad YNTP", style);
            createCell(row1, 5, "Contrato", style);
            createCell(row1, 6, "NIT Contraparte", style);
            createCell(row1, 7, "Valor", style);
            createCell(row1, 8, "Cod País", style);
            createCell(row1, 9, "País", style);
            createCell(row1, 10, "Cuenta Local", style);
            createCell(row1, 11, "Periodo", style);
            createCell(row1, 12, "Fuente", style);
            createCell(row1, 13, "Input", style);
            createCell(row1, 14, "Componente", style);
        }
    ...
    private void writeDataLines(List<IntergrupoV1> intListN){
        int rowCount = 1;
        ...
        for(IntergrupoV1 inter : interList){
        ...
        {
            rowCount = 1;
            for(IntergrupoV1 inter : intListN){
    ...
    public void export(HttpServletResponse response,List<IntergrupoV1> intListN) throws IOException {
        writeHeaderLine(intListN);
        writeDataLines(intListN);
    ...
    private void writeDataLinesFinal(List<IntergrupoV1Final> intList){
        int rowCount = 1;
        ...
        for(IntergrupoV1Final inter : intList){
            Row row = sheet.createRow(rowCount++);
            int columnCount = 0;
    ...
    public void exportFinal(HttpServletResponse response,List<IntergrupoV1Final> intList) throws IOException {
        writeHeaderLine(null);
    ...
    private void writeHeaderLineLog(){
        sheet = workbook.createSheet("Log Intergrupo V1");
        Row row = sheet.createRow(0);
