...
public class RoleViewService {
    ...
    public List<View> findViewsVer(int idRol){
        Query consulta = entityManager.createNativeQuery("select b.* from (SELECT * FROM preciso_administracion_rol_vista where id_perfil = ? and p_visualizar=1) a\n" +
                "\t\t inner join preciso_administracion_vistas b on a.id_vista=b.id_vista ", View.class);
        consulta.setParameter(1,idRol);
        return consulta.getResultList();
    }

    public List<View> findViewsModificar(int idRol){
        Query consulta = entityManager.createNativeQuery("select b.* from (SELECT * FROM preciso_administracion_rol_vista where id_perfil = ? and p_modificar=1) a\n" +
                "\t\t inner join preciso_administracion_vistas b on a.id_vista=b.id_vista ", View.class);
        consulta.setParameter(1,idRol);
        return consulta.getResultList();
    }
