...
public class UVRIcrfListReport {
    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private List<String[]> dataStringList;
    private List<UVRIcrf> listDataObject;

    private static final long serialVersionUID = 1L;
    private static final int BYTES_DOWNLOAD = 1024;

    public UVRIcrfListReport(List<String[]> dataStringList, List<UVRIcrf> listDataObject) {
    ...
    private void writeHeaderLine(){
        sheet = workbook.createSheet("UVR ICRF");
        ...
        int consecutive= 0;
        createCell(row, consecutive++, "Fecha (dd/mm/aaaa)", style);
        createCell(row, consecutive++, "Pesos colombianos por UVR", style);
        createCell(row, consecutive++, "Variación anual porcentual %", style);
    }
    ...
    private void writeDataLines(){
        ...
        style1.setDataFormat(workbook.createDataFormat().getFormat("dd/mm/yyyy"));
        ...
        for(UVRIcrf data: listDataObject){
            Row row = sheet.createRow(rowCount++);
            int columnCount = 0;
            createCell(row,columnCount++,data.getFecha(),style1);
            createCell(row,columnCount++,data.getPesoCopUvr(),style2);
            createCell(row,columnCount++,data.getVariacionAnual(),style2);
        }
    }
