...
public class ViewService {
    ...
    public List<View> findAll(){
        Query query = entityManager.createNativeQuery("SELECT * FROM nexco_vistas ORDER BY nombre", View.class);
        return query.getResultList();
    }

    public List<String> findAllPrincipal(){
        Query query = entityManager.createNativeQuery("SELECT nv.menu_principal FROM nexco_vistas AS nv GROUP BY nv.menu_principal");
        return query.getResultList();
    }
    ...
    public List<View> findByMenuPrincipal(String menu_principal){
        /*
        String queryString = "SELECT * FROM nexco_vistas WHERE menu_principal = :menu_principal ORDER BY nombre";
        if (menu_principal == null) {
            queryString = "SELECT * FROM nexco_vistas ORDER BY nombre";
        }
