...
public class Contnv15ListReport {

    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private List<Contnv15> desconnv15List;

    public Contnv15ListReport(List<Contnv15> desconnv15List){
    ...
    private void writeDataLines(){
        ...
        for(Contnv15 desconnv15: desconnv15List){
            Row row = sheet.createRow(rowCount++);
            int columnCount = 0;

            createCell(row,columnCount++,desconnv15.getCuenta(),style);
            createCell(row,columnCount++,desconnv15.getDivisa(),style);
            createCell(row,columnCount++,desconnv15.getSald().toString(),style1);
            createCell(row,columnCount++,desconnv15.getSalc().toString(),style1);
            createCell(row,columnCount++,desconnv15.getSaldDiv().toString(),style1);
            createCell(row,columnCount++,desconnv15.getSalcDiv().toString(),style1);
