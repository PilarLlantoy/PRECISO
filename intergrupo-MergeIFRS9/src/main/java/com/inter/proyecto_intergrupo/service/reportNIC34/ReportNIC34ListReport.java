...
public class ReportNIC34ListReport {
    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private XSSFSheet sheet2;
    private List<Object[]> nic34List;
    private List<BaseNIC34> listNic34;
    private List<Object[]> listBalance;
    private static final long serialVersionUID = 1L;
    private static final int BYTES_DOWNLOAD = 1024;

    public ReportNIC34ListReport(List<Object[]> nic34List, List<BaseNIC34> listNic34, List<Object[]> listBalance) {
    ...
    private void writeHeaderLineBalance(List<String> listFechas)
    {
        CellStyle style = workbook.createCellStyle();
        XSSFFont font = workbook.createFont();
        ...
        if(listFechas.isEmpty()) {
            ...
            createCell(row, columnCountH++, listFechas.get(0), style);
            createCell(row, columnCountH++, listFechas.get(1), style);
        }
    ...
    private void writeHeaderLineNotas(List<String> listFechas)
    {
        CellStyle style = workbook.createCellStyle();
        XSSFFont font = workbook.createFont();
        font.setColor(IndexedColors.WHITE.getIndex());
        ...
        createCell(row, columnCountH++, "Grupo", style);
        createCell(row, columnCountH++, "Nota", style);
        createCell(row, columnCountH++, "Saldo "+listFechas.get(0), style);
        if(listFechas.size() <= 2)
            createCell(row, columnCountH++, "Saldo "+listFechas.get(1), style);
        if(listFechas.size() > 2)
            createCell(row, columnCountH++, "Saldo "+listFechas.get(2), style);
        createCell(row, columnCountH++,"Variación 1", style);
        if(listFechas.size() > 2) {
            createCell(row, columnCountH++, "Variación 2", style);
            createCell(row, columnCountH++, "Saldo "+listFechas.get(1), style);
            createCell(row, columnCountH++, "Saldo "+listFechas.get(3), style);
        }
    ...
    private void writeHeaderLinePyg(List<String> listFechas)
    {
        CellStyle style = workbook.createCellStyle();
        XSSFFont font = workbook.createFont();
        font.setColor(IndexedColors.WHITE.getIndex());
        ...
        if(listFechas.isEmpty()) {
            ...
            createCell(row, columnCountH++, listFechas.get(0), style);
            createCell(row, columnCountH++, listFechas.get(2), style);
        }
        createCell(row, columnCountH++,"Variación 1", style);
        createCell(row, columnCountH++,"Variación 2", style);
        if(listFechas.isEmpty()) {
            ...
            createCell(row, columnCountH++, listFechas.get(1), style);
            createCell(row, columnCountH++, listFechas.get(3), style);
        }
    ...
    private void writeDataLines(){
        ...
        for(BaseNIC34 paramNIC34: listNic34){
    ...
    private void writeDataLinesNotas(List<String> listFechas){
        int rowCount = 1;
    ...
    public void exportBalance(HttpServletResponse response,List<String> listFechas) throws IOException {
        writeHeaderLineBalance(listFechas);
    ...
    public void exportPyg(HttpServletResponse response,List<String> listFechas) throws IOException {
        writeHeaderLinePyg(listFechas);
        writeDataLinesPyg();
    ...
    public void exportNotas(HttpServletResponse response, List<String> listFechas) throws IOException {
        writeHeaderLineNotas(listFechas);
        writeDataLinesNotas(listFechas);
