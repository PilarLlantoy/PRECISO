...
public class RysParametricListReport {
    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private List<RysParametric> rysParametricList;
    private List<Object[]> rysParametricListObject;

    public RysParametricListReport(List<RysParametric> rysParametricList){
        this.rysParametricList = rysParametricList;
        workbook = new XSSFWorkbook();

    }

    public RysParametricListReport(List<Object[]> rysParametricListObject, int number){
        this.rysParametricListObject = rysParametricListObject;
        workbook = new XSSFWorkbook();

    }

    private void writeHeaderLine(){
        sheet = workbook.createSheet("Parametrica RYS");
        ...
        createCell(row,	3, "Cuenta PYG", style);
        createCell(row,	4, "Cuenta Neocon", style);
        createCell(row,	5, "Cuenta Neocon PYG", style);
    }
    ...
    private void writeDataLines(){
        ...
        for(RysParametric account: rysParametricList){
            Row row = sheet.createRow(rowCount++);
            int columnCount = 0;

            createCell(row,columnCount++,	account.getCodigo()	,style);
            createCell(row,columnCount++,	account.getCodigoNombre()	,style);
            createCell(row,columnCount++,	account.getCuenta()	,style);
            createCell(row,columnCount++,	account.getCuentaPyg()	,style);
            createCell(row,columnCount++,	account.getCuentaNeocon()	,style);
            createCell(row,columnCount++,	account.getCuentaNeoconPyg()	,style);
        }
    }
