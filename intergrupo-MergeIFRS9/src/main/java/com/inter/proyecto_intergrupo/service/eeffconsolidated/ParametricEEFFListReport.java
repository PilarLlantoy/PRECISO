...
public class ParametricEEFFListReport {
    private XSSFWorkbook workbook;
    private XSSFSheet sheet;
    private List<String[]> parametriceeff;
    private List<ParametricEEFF> listAccount1;
    private static final long serialVersionUID = 1L;
    private static final int BYTES_DOWNLOAD = 1024;

    public ParametricEEFFListReport(List<String[]> parametriceeff, List<ParametricEEFF> listAccount) {
    ...
    private void writeHeaderLine(){
        ...
        createCell(row, 0, "Parametro", style);
        createCell(row, 1, "Concepto", style);
        createCell(row, 2, "Cuenta", style);
        createCell(row, 3, "Cuenta Aux", style);
        createCell(row, 4, "Porcentaje", style);
    }
    ...
    private void writeDataLines(){
        ...
        for(ParametricEEFF accountCc: listAccount1){
            Row row = sheet.createRow(rowCount++);
            int columnCount = 0;
            createCell(row,columnCount++,accountCc.getParametro(),style);
            createCell(row,columnCount++,accountCc.getConcepto(),style);
            createCell(row,columnCount++,accountCc.getCuenta(),style);
            createCell(row,columnCount++,accountCc.getCuenta2(),style);
            createCell(row,columnCount++,accountCc.getPorcentaje(),style1);
